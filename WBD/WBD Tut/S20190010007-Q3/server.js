...
//Get Individual
app.get('/ClimateDetails/:id',(req,res)=>{
    // const sql = "Select * from Entry WHERE pid=?"
    db.all("Select * from Entry WHERE pid=?",[req.params.id],(err,rows)=>{
        if(err){
            return console.log(error)
        }
        res.send(rows)
...
//Delete Individual
app.delete('/ClimateDetails/:id',(req,res)=>{
    db.all("DELETE from Entry WHERE pid=?",[req.params.id],(err,rows)=>{
        if(err){
            return console.log(error)
        }
